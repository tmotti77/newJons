    ...
    <Screen style={{ gap: spacing.xl, justifyContent: "center" }}>
      ...
      <Card style={{ gap: spacing.m }}>
        {ranked.map((p, i) => {
          ...
          return (
            <View key={p.playerId} style={{ flexDirection: "row", alignItems: "center", gap: spacing.m }}>
              <Text style={[type.h2, { width: 24 }]}>{i + 1}.</Text>
              <Avatar id={p.avatar} size={36} />
              <Text style={[type.body, { flex: 1 }]}>{p.displayName}</Text>
              <Text style={type.dim}>
                ₪{netWorth(st, cryptoPrice)} · 😊{st.happiness} · 🎓{totalCourses(st)} · 💼T{Math.max(0, st.jobTier)}
              </Text>
            </View>
          );
        })}
      ...
      <Button label={t("over.playAgain")} onPress={home} />
    </Screen>
  );
}
