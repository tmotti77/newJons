...
- Art: flat, bold, slightly grotesque cartoon characters (think _Gartic Phone_ / _BitLife_ energy), bright colors, big readable numbers. Town = single stylized map screen with tappable buildings.
...
- **Server is authoritative.** Clients send _plans_ (intent), never results. The `resolve-round` edge function is the ONLY code path that mutates game state.
...
_Last updated: 2026-07-19 · v1.4 (Phases 0–3 complete + emulator-verified end-to-end incl. critical RLS fix; Phase 3 acceptance pending only the physical 2+ phone playtest — use `scripts/join-lobby-bot.ts` to fill seats) · Next review: after first real playtest_
